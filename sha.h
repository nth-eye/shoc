...
constexpr uint32_t Sigma256_0(uint32_t x)   { return ror(x, 7)  ^ ror(x, 18) ^ (x >> 3);  }
constexpr uint32_t Sigma256_1(uint32_t x)   { return ror(x, 17) ^ ror(x, 19) ^ (x >> 10); }
constexpr uint64_t Sigma512_0(uint64_t x)   { return ror(x, 1)  ^ ror(x, 8)  ^ (x >> 7);  }
constexpr uint64_t Sigma512_1(uint64_t x)   { return ror(x, 19) ^ ror(x, 61) ^ (x >> 6);  }
constexpr uint32_t Sum256_0(uint32_t x)     { return ror(x, 2)  ^ ror(x, 13) ^ ror(x, 22); }
constexpr uint32_t Sum256_1(uint32_t x)     { return ror(x, 6)  ^ ror(x, 11) ^ ror(x, 25); }
constexpr uint64_t Sum512_0(uint64_t x)     { return ror(x, 28) ^ ror(x, 34) ^ ror(x, 39); }
constexpr uint64_t Sum512_1(uint64_t x)     { return ror(x, 14) ^ ror(x, 18) ^ ror(x, 41); }
...
private:
    ...
    #define SHA_2_ROUND(ver)                                                                                    \
        for (int t = 16; t < N_WORDS; ++t)                                                                      \
            w[t] = Sigma##ver##_1(w[t-2]) + w[t-7] + Sigma##ver##_0(w[t-15]) + w[t-16];                         \
        for (int t = 0; t < N_WORDS; ++t) {                                                                     \
            word tmp1 = var[h] + Sum##ver##_1(var[e]) + Ch(var[e], var[f], var[g]) + SHA_##ver##_K[t] + w[t];   \
            word tmp2 = Sum##ver##_0(var[a]) + Maj(var[a], var[b], var[c]);                                     \
            var[h] = var[g];                                                                                    \
            var[g] = var[f];                                                                                    \
            var[f] = var[e];                                                                                    \
            var[e] = var[d] + tmp1;                                                                             \
