...
{
    hmac_ctx<H> ctx;
    byte bin[H::hash_size];
    char str[H::hash_size * 2 + 1];

    for (auto it : test) {
        hmac<H>(it.key, it.msg, bin); // free function test
        utl::bin_to_cstr(bin, sizeof(bin), str, sizeof(str));
        ASSERT_STREQ(it.exp.data(), str);
        ctx(it.key, it.msg, bin); // context test
        utl::bin_to_cstr(bin, sizeof(bin), str, sizeof(str));
        ASSERT_STREQ(it.exp.data(), str);
    }
}
