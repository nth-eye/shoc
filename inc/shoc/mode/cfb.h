 ...
 */
template<class E>
constexpr void cfb_encrypt(span_i<E::key_size> key, span_i<E::block_size> iv, const byte* in, byte* out, size_t len)
{
    E ciph {key};
 ...
 */
template<class E>
constexpr void cfb_decrypt(span_i<E::key_size> key, span_i<E::block_size> iv, const byte* in, byte* out, size_t len)
{
    E ciph {key};
    byte buf[E::block_size];
    copy(buf, iv.data(), iv.size());
